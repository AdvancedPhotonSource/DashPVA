...
**Input alignment.** All input widgets (`QSpinBox`, `QDoubleSpinBox`, `QLineEdit`, editable combo boxes) must right-align their text — set `alignment` to `Qt::AlignRight|Qt::AlignVCenter` in the `.ui` (QSS cannot align a spin box's editor). Give numeric fields a fixed width too, so the row does not shift as the digit count changes.
...
- **Docks own their panel.** A dock computes and formats its own content. `Info2DDock` owns 2D-info presentation (dims, current-frame details, pixel readout), `Info3DDock` owns 3D info (HKL/volume/slice), `ROICalcDock` owns ROI math, the Area Detector's waterfall/stats docks own their own analysis, etc. A change to "how 2D info is shown" touches `info_2d_dock.py` alone.
