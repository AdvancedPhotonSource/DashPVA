...
- **No inline config, no inline constants.** Every constant belongs in `settings.py` — not just config-driven values (paths, PV names, thresholds) but also static tuning constants (rates, limits, sizes, timeouts). Do not define a module- or class-level constant in a viewer or utility module; add it to `settings.py` (module-level static, like `MAX_PROCESS_RATE_HZ` / `HDF5_STRUCTURE`, when not config-driven) and read it via `import dashpva.settings as app_settings`.
