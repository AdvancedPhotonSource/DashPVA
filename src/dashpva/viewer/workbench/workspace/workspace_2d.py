...
playback, intensity/levels handling, hover readout and ROI hooks. Loaded from
``workspace_2d.ui`` and registered as its own
...
class Workspace2D(BaseTab):
    ...
    def connect_all(self):
        ...
        try:
            ...
            if hasattr(self, 'image_view') and self.image_view is not None:
                self.image_view.sigTimeChanged.connect(self._on_timeline_changed)
    ...
    def _compute_levels_arg(self):
        """Explicit (lo, hi) display-domain levels for manual mode, or None when Auto Levels is on.

        Frame-independent: derived from the manual-override caches or the Vmin/Vmax
        spinboxes (log-mapped when log scale is active), so it applies to every frame.
        """
        auto_levels = hasattr(self, 'cbAutoLevels') and self.cbAutoLevels.isChecked()
        if auto_levels:
            return None
    ...
    def _display_stack(self):
        """Hand the whole 3D stack to the ImageView so its native timeline scrubs frames."""
        try:
            data = getattr(self, 'current_2d_data', None)
            if data is None or getattr(data, 'ndim', None) != 3 or not hasattr(self, 'image_view'):
                return
            disp = self._apply_display_transform(data)
            auto_levels = hasattr(self, 'cbAutoLevels') and self.cbAutoLevels.isChecked()
            auto_range = bool(getattr(self, '_force_auto_range_next', False))
            levels_arg = self._compute_levels_arg()
            xvals = np.arange(data.shape[0], dtype=float)
            try:
                if levels_arg is not None:
                    self.image_view.setImage(disp, xvals=xvals, autoLevels=False, autoRange=auto_range, autoHistogramRange=False, levels=levels_arg)
                else:
                    self.image_view.setImage(disp, xvals=xvals, autoLevels=auto_levels, autoRange=auto_range, autoHistogramRange=auto_levels)
            finally:
                try:
                    ...
                    pass
            try:
                self._apply_levels_for_current_mode(disp[int(self.image_view.currentIndex)])
            except Exception:
                pass
        except Exception:
            pass

    def _show_frame_via_timeline(self, frame_index):
        """Point the ImageView's native timeline at frame_index and re-apply manual levels (no re-feed)."""
        try:
            iv = getattr(self, 'image_view', None)
            if iv is None:
                return
            try:
                if int(iv.currentIndex) != int(frame_index):
                    iv.setCurrentIndex(int(frame_index))
            except Exception:
                pass
            levels_arg = self._compute_levels_arg()
            if levels_arg is not None:
                try:
                    iv.setLevels(*levels_arg)
                except Exception:
                    pass
        except Exception:
            pass

    def _on_timeline_changed(self, ind, time=None):
        """Native ImageView timeline scrubbed: drive the frame spinbox (single source of truth)."""
        try:
            idx = int(round(float(ind)))
            if hasattr(self, 'frame_spinbox') and self.frame_spinbox.isEnabled() and self.frame_spinbox.value() != idx:
                self.frame_spinbox.setValue(idx)
        except Exception:
            pass

    def _refresh_current_frame_image(self, frame_data: np.ndarray = None):
        """Refresh the displayed image and sync levels/histogram.

        For 3D stacks the whole stack already lives in the ImageView (native
        timeline owns frame display), so this only re-applies levels for the
        current frame; for 2D data it sets the single image.
        ...
        try:
            if not hasattr(self, 'image_view') or self.image_view is None:
                return
            # 3D: stack is owned by the ImageView; only re-apply levels for the current frame
            if getattr(self, 'current_2d_data', None) is not None and getattr(self.current_2d_data, 'ndim', None) == 3:
                levels_arg = self._compute_levels_arg()
                if levels_arg is not None:
                    try:
                        self.image_view.setLevels(*levels_arg)
                    except Exception:
                        pass
                try:
                    frame = self.get_current_frame_data()
                    if frame is not None:
                        self._apply_levels_for_current_mode(self._apply_display_transform(frame))
                except Exception:
                    pass
                return
            # Resolve frame (2D path)
            try:
    ...
    def display_2d_data(self, data):
        ...
        try:
            ...
            try:
                ...
                pass
            if data.ndim == 3:
                # Feed the whole stack so the ImageView's native timeline scrubs frames
                self._display_stack()
            else:
    ...
    def on_frame_spinbox_changed(self, frame_index):
        ...
        try:
            ...
            # Move the ImageView's native timeline to this frame (stack already loaded)
            self._show_frame_via_timeline(frame_index)
    ...
    def on_log_scale_toggled(self, checked):
        ...
        try:
            if hasattr(self, 'image_view') and hasattr(self, 'current_2d_data') and self.current_2d_data is not None:
                # Log scale changes the display transform: re-feed the stack (3D) or the single frame (2D)
                if self.current_2d_data.ndim == 3:
                    self._display_stack()
                else:
                    self._refresh_current_frame_image()
