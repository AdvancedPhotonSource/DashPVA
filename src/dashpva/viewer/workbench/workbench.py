...
from dashpva.viewer.controls.controls_1d import Controls1D
...
class WorkbenchWindow(BaseWindow):
    ...
    def __init__(self):
        ...
        # 2D controls are wired inside Workspace2D (see workspace/workspace_2d.py)
        # ======== CONTROLS END ======= #
        ...
        # Create the 2D workspace tab (owns the 2D viewer, controls and playback)
        self.tab_2d = Workspace2D(parent=self, main_window=self)
        try:
            # Keep the 2D tab first in the bar and selected by default
            bar = self.tabWidget_analysis.tabBar()
            bar.moveTab(self.tabWidget_analysis.indexOf(self.tab_2d), 0)
            self.tabWidget_analysis.setCurrentWidget(self.tab_2d)
        except Exception:
            pass
    ...
    # let ROI managers/docks and legacy callers keep using self.<attr> unchanged.
    @property
    def image_view(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.image_view if tab is not None else None

    @property
    def plot_item(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.plot_item if tab is not None else None

    @property
    def frame_spinbox(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.frame_spinbox if tab is not None else None
    ...
    def axis_2d_x(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.axis_2d_x if tab is not None else None

    @property
    def axis_2d_y(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.axis_2d_y if tab is not None else None
    ...
    def clear_2d_plot(self):
        if getattr(self, 'tab_2d', None) is not None:
            return self.tab_2d.clear_2d_plot()

    def get_current_frame_data(self):
        tab = getattr(self, 'tab_2d', None)
        return tab.get_current_frame_data() if tab is not None else None

    def start_playback(self):
        if getattr(self, 'tab_2d', None) is not None:
            return self.tab_2d.start_playback()

    def open_roi_2d_plot_dock(self, roi):
        if getattr(self, 'tab_2d', None) is not None:
            return self.tab_2d.open_roi_2d_plot_dock(roi)

    def _select_2d_tab(self):
        try:
            if getattr(self, 'tab_2d', None) is not None:
                self.tabWidget_analysis.setCurrentWidget(self.tab_2d)
        except Exception:
    ...
    def open_roi_plot(self, roi):
        ...
        try:
            ...
            try:
                from dashpva.viewer.workbench.rois.roi_plot_dialog import ROIPlotDialog
            except Exception:
                ROIPlotDialog = None
    ...
    def open_roi_math_dock(self, roi):
        ...
        try:
            ...
            try:
                from dashpva.viewer.workbench.rois.roi_math_dock import ROIMathDock
            except Exception:
                ROIMathDock = None
    ...
    def open_roi_plot_dock(self, roi):
        ...
        try:
            ...
            if x0 >= x1 or y0 >= y1:
                QMessageBox.information(self, "ROI Plot", "ROI area is empty or out of bounds.")
                return
            frame_data[y0:y1, x0:x1]
            # Create and add the dock widget
            try:
                from dashpva.viewer.workbench.rois.roi_plot_dock import ROIPlotDock
            except Exception:
                ROIPlotDock = None
    ...
    def visualize_selected_dataset(self):
        ...
        try:
            ...
            # Reset the 2D view's ROI-axis overlay for any non-ROI path
            try:
                if self.tab_2d is not None:
                    self.tab_2d.reset_roi_axis()
            except Exception:
                pass
            ...
            if is_image_data:
                ...
                with h5py.File(self.current_file_path, 'r') as h5f:
                    ...
                    # Read origin attributes for the axis toggle
                    try:
                        _roi_w = data.shape[-1] if data.ndim >= 1 else 1
                        _roi_h = data.shape[-2] if data.ndim >= 2 else 1
                        roi_origin = (
                            int(dset.attrs.get('x', 0)),
                            int(dset.attrs.get('y', 0)),
                            int(dset.attrs.get('w', _roi_w)),
                            int(dset.attrs.get('h', _roi_h)),
                        )
                    except Exception:
                        roi_origin = None
                ...
                if data.ndim >= 2 and np.issubdtype(data.dtype, np.number):
                    ...
                    try:
                        if self.tab_2d is not None:
                            self.tab_2d.set_roi_axis_origin(roi_origin)
                    except Exception:
                        pass
    ...
    def _play_folder_section_stack(self, item):
        ...
        try:
            ...
            try:
                from dashpva.viewer.tools.file_convert import list_images, stack_images
            except Exception:
    ...
    def remove_folder_section(self, folder_item):
        ...
        """
        folder_item.data(0, Qt.UserRole + 1)
        folder_name = folder_item.text(0)
