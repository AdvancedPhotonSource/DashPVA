...
class TestDefaultProfileDataLiteral:
    ...
    def test_roi_and_stats_seeded(self):
        """ROI/STATS are seeded so the GUI shows the canonical sections."""
        from dashpva.scripts.seed_profile_defaults_sql import DEFAULT_PROFILE_DATA

        roi = DEFAULT_PROFILE_DATA["ROI"]
        assert set(roi.keys()) == {"ROI1", "ROI2", "ROI3", "ROI4"}
        for r in roi.values():
            assert set(r.keys()) == {"MIN_X", "MIN_Y", "SIZE_X", "SIZE_Y"}

        stats = DEFAULT_PROFILE_DATA["STATS"]
        assert set(stats.keys()) == {"STATS1", "STATS2", "STATS3", "STATS4", "STATS5"}
        for s in stats.values():
            assert set(s.keys()) == {"TOTAL", "MIN", "MAX", "SIGMA", "MEAN"}
