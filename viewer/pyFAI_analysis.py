...
# from scipy import ndimage  # replacing cv2 for image processing
...
#     # Define a signal for thread-safe image updates
...
#         self.new_image_signal.connect(self.update_image)
...
#         self.max_frames_spinbox.setValue(100)
...
#         and then emits a signal to process it in the GUI thread.
#         """
#         if self.paused:
...
#                         # Emit signal instead of direct call to update_image
...
#         This method runs in the GUI thread thanks to the signal-slot connection.
#         """
#         if self.ai is None:
...
#                         return
#                 elif image.ndim == 3:
#                     # For example: Convert colored (RGB) 3D images to grayscale by averaging channels (using numpy instead of cv2)
...
#             # If a mask is provided, ensure its shape matches - using scipy instead of cv2
...
#                     # Use scipy.ndimage to resize the mask
#                     scale_y = image.shape[0] / self.mask.shape[0]
#                     scale_x = image.shape[1] / self.mask.shape[1]
#                     # Convert to float for interpolation
#                     mask_float = self.mask.astype(np.float32)
#                     # Use zoom to resize
#                     resized_mask = ndimage.zoom(mask_float, (scale_y, scale_x), order=0)
#                     # Convert back to boolean
#                     self.mask = resized_mask > 0.5
...
#     # Set environment variables to help Qt find plugins
#     import os
#     # This ensures Qt looks in the right place for plugins
