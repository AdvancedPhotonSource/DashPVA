...
class HpcAdMetadataProcessor(AdImageProcessor):
    def __init__(self, configDict={}):
        ...
        # Current metadata map: Stores the most recent metadata for each channel
...
#    def configure(self, configDict):
    ...
    def associateMetadata(self, mdChannel, frameId, frameAttributes):
    ...
    def process(self, pvObject):
        ...
        # Process each metadata channel regardless of the timestamp of the frame
        for metadataChannel, metadataQueue in self.metadataQueueMap.items():
            # Get the latest metadata available for the channel
            if not metadataQueue:
                latest_metadata = metadataQueue.get_nowait()
                self.currentMetadataMap[metadataChannel] = latest_metadata
                associationResult = self.associateMetadata(metadataChannel, frameId, frameAttributes)
                if not associationResult:
                    self.nFrameErrors += 1
